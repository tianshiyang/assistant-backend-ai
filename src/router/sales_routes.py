#!/user/bin/env python
# -*- coding: utf-8 -*-
"""
@Time    : 2026/2/15 18:19
@Author  : tianshiyang
@File    : sales_routes.py
"""
from flask import Blueprint

from handler.sales_handler import get_sales_by_id_handler, get_all_sales_handler, update_sales_handler, get_all_sales_no_pagination_handler

sales_blueprint = Blueprint("sales_router", __name__, url_prefix="")

# 获取销售详情信息
sales_blueprint.add_url_rule("/api/manage/sales/detail_by_id", view_func=get_sales_by_id_handler)

# 查询所有销售
sales_blueprint.add_url_rule("/api/manage/sales/get_all_sales", view_func=get_all_sales_handler)

# 查询所有销售（不分页）
sales_blueprint.add_url_rule("/api/manage/sales/all_no_pagination", view_func=get_all_sales_no_pagination_handler)

# 新增销售
sales_blueprint.add_url_rule("/api/manage/sales/update_sales", methods=["POST"], view_func=update_sales_handler)
